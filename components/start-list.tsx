...
export function StartList({ competitionId, selectedId, onSelect, readOnly = false, showAdminChanges = false, adminReorder = false, compact = false, dense = false, organizationId, hideWithdrawn = false, orderedIds, onReorder }: { competitionId: string; selectedId?: string; onSelect?: (entry: StartEntry) => void; readOnly?: boolean; showAdminChanges?: boolean; adminReorder?: boolean; compact?: boolean; dense?: boolean; organizationId?: string; hideWithdrawn?: boolean; orderedIds?: string[]; onReorder?: (sourceId: string, targetId: string) => void }) {
  ...
  return <div className={`flex flex-col ${compact?(dense?"gap-1 lg:gap-0.5":"gap-1"):"gap-3"}`}>{entries.map((e,index)=>{const displayOrder=e.order,player=getPlayer(e.playerId),horse=getHorse(e.horseId),org=e.organizationId?getOrg(e.organizationId):horse?getOrg(horse.orgId):undefined,selected=selectedId===e.id,Tag=readOnly?"div":"button";return <Tag key={e.id} data-start-entry-id={e.id} data-start-reorderable={adminReorder&&!e.withdrawn?"true":undefined} {...(readOnly?{}:{type:"button" as const,onClick:()=>onSelect?.(e),"aria-pressed":selected})} onTouchStart={adminReorder&&onReorder&&!e.withdrawn?event=>{if((event.target as Element).closest("button")||event.touches.length!==1)return;const touch=event.touches[0],row=event.currentTarget;touchOrigin.current={x:touch.clientX,y:touch.clientY};holdTimer.current=window.setTimeout(()=>{holdTimer.current=null;beginDrag(e,row,touch.clientX,touch.clientY)},180)}:undefined} onTouchMove={adminReorder&&onReorder?event=>{const origin=touchOrigin.current,touch=event.touches[0];if(origin&&touch&&Math.hypot(touch.clientX-origin.x,touch.clientY-origin.y)>8)cancelHold()}:undefined} onTouchEnd={adminReorder&&onReorder?cancelHold:undefined} onPointerDown={adminReorder&&onReorder&&!e.withdrawn?event=>{if(event.pointerType==="touch"||event.button!==0||(event.target as Element).closest("button"))return;event.preventDefault();beginDrag(e,event.currentTarget,event.clientX,event.clientY)}:undefined} onContextMenu={adminReorder&&onReorder?event=>event.preventDefault():undefined} className={`flex items-center text-left transition ${readOnly?"":"active:scale-[0.99]"} ${compact?(dense?"min-h-12 gap-2 rounded-lg border px-2 py-1 lg:min-h-6 lg:gap-1 lg:rounded-md lg:px-1.5 lg:py-0":"min-h-12 gap-2 rounded-lg border px-2 py-1"):"min-h-24 gap-4 rounded-2xl border-2 px-5 py-3 shadow-sm"} ${selected?"border-primary bg-primary/10 ring-4 ring-primary/25":"border-border bg-card"} ${draggingId===e.id?"border-primary border-dashed bg-primary/10 opacity-40":""} ${hoveringId===e.id?"border-primary bg-blue-50 ring-2 ring-primary/50":""} ${adminReorder&&onReorder&&!e.withdrawn?"select-none cursor-grab":""} ${readOnly?"":"hover:border-primary"}`}>
    ...
    <span className={`flex min-w-0 flex-1 ${compact?(dense?"flex-col gap-1 sm:flex-row sm:items-center sm:gap-2 lg:gap-1":"flex-col gap-1 sm:flex-row sm:items-center sm:gap-2"):"flex-col"}`}><span className={`${compact?(dense?"flex min-w-0 flex-wrap items-center gap-1 lg:flex-nowrap lg:gap-1":"flex min-w-0 flex-wrap items-center gap-1"):"mb-1 flex flex-wrap items-center gap-2"}`}><span className={`font-bold ${compact?(dense?"truncate text-base lg:text-xs":"truncate text-base"):"text-2xl"} ${e.withdrawn?"text-destructive":"text-foreground"}`}>{player?.name??"—"} {e.withdrawn&&"【棄権】"}</span>{riderGaps.has(e.id)&&<span className={`shrink-0 rounded-md bg-amber-100 font-bold text-amber-950 ${compact?(dense?"px-1.5 py-0.5 text-xs lg:px-1 lg:py-0 lg:text-[10px]":"px-1.5 py-0.5 text-xs"):"px-2.5 py-1 text-base"}`}>選手：間に{riderGaps.get(e.id)}頭（要確認）</span>}{e.needsAffiliationReview&&<span className={`shrink-0 rounded-md bg-amber-100 font-bold text-amber-950 ${compact?(dense?"px-1.5 py-0.5 text-xs lg:px-1 lg:py-0 lg:text-[10px]":"px-1.5 py-0.5 text-xs"):"px-2.5 py-1 text-base"}`}>所属要確認</span>}{showAdminChanges&&e.adminChangeMark&&<span className={`shrink-0 rounded-md font-bold ${compact?(dense?"px-1.5 py-0.5 text-xs lg:px-1 lg:py-0 lg:text-[10px]":"px-1.5 py-0.5 text-xs"):"px-2.5 py-1 text-base"} ${e.adminChangeMark==="added"?"bg-primary text-primary-foreground":"bg-violet-700 text-white"}`}>{e.adminChangeMark==="added"?"追加":"変更反映"}</span>}</span><span className={`flex min-w-0 flex-1 flex-wrap items-center gap-1 ${compact?(dense?"text-sm lg:text-xs":"text-sm"):"text-xl"} ${e.withdrawn?"text-destructive":"text-foreground"}`}><span className="min-w-0 truncate">{compact?horse?.name??"—":`馬：${horse?.name??"—"}`}</span>{horseGaps.has(e.id)&&<span className={`shrink-0 rounded-md bg-sky-100 font-bold text-sky-950 ${compact?(dense?"px-1.5 py-0.5 text-xs lg:px-1 lg:py-0 lg:text-[10px]":"px-1.5 py-0.5 text-xs"):"px-2.5 py-1 text-base"}`}>馬：間に{horseGaps.get(e.id)}頭（要確認）</span>}</span><span className={`${compact?(dense?"hidden xl:block max-w-48 truncate text-xs lg:text-[11px]":"hidden xl:block max-w-48 truncate text-xs"):"text-lg"} text-muted-foreground`}>{org?.name??"—"}</span></span>
    {adminReorder&&<span className={`flex shrink-0 items-center ${compact?"gap-1":"gap-2"}`}>
      {!e.withdrawn&&onReorder&&<button type="button" aria-label={`${displayOrder}番 ${player?.name??""}を押したまま移動`} title="押したまま上下に動かす" onTouchStart={event=>{event.stopPropagation();const touch=event.touches[0];if(!touch)return;beginDrag(e,event.currentTarget.closest<HTMLElement>("[data-start-entry-id]"),touch.clientX,touch.clientY)}} onPointerDown={event=>{event.stopPropagation();if(event.pointerType==="touch"||event.button!==0)return;event.preventDefault();beginDrag(e,event.currentTarget.closest<HTMLElement>("[data-start-entry-id]"),event.clientX,event.clientY)}} onContextMenu={event=>event.preventDefault()} className={`flex touch-none select-none items-center justify-center rounded-md border-2 border-primary/50 bg-primary/10 text-primary ${draggingId===e.id?"ring-2 ring-primary":""} ${compact?(dense?"size-11 lg:size-6":"size-11"):"size-12"}`}><GripVertical className={dense?"size-6 lg:size-4":"size-6"}/></button>}
