...
export function SettlementPanel() {
  ...
  useEffect(() => {
    loadReceptionPayments().then(rows => rows.forEach(r => setPayment(r.organization_id, r.paid))).catch(e => setError(e instanceof Error ? e.message : "入金情報を読み込めませんでした"))
  }, [setPayment])
  ...
  return <div className="flex flex-col gap-5">
    ...
    {rows.map(r => <div key={r.orgId} className="rounded-2xl border-2 border-border bg-card p-5 shadow-sm">
      ...
      <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3">
        <Cell label="通常エントリー料金" value={formatYen(r.normalEntry)} /><Cell label="追加料金" value={formatYen(r.additional)} /><Cell label="変更料金" value={formatYen(r.change)} /><Cell label="競技変更の差額" value={formatYen(r.competitionDiff)} /><Cell label="請求合計（最終精算金額）" value={formatYen(r.total)} strong />
      </dl>
