...
export function RequestPanel() {
  ...
  async function reload() {
    ...
    try { setRequests(await loadReceptionRequests()) }
    catch (e) { setError(e instanceof Error ? e.message : "申請を読み込めませんでした") }
  ...
  return <div className="flex flex-col gap-4">
    <div className="flex flex-wrap items-center gap-3 rounded-2xl border-2 border-border bg-card p-4">
      <div><p className="text-lg font-semibold text-muted-foreground">未反映の受付申請</p><p className="text-3xl font-bold text-foreground">{pendingCount}件</p></div>
      <button onClick={() => void reload()} disabled={loading} className="ml-auto flex items-center gap-2 rounded-xl border-2 border-border px-4 py-3 text-lg font-bold disabled:opacity-50"><RefreshCw className={`size-5 ${loading ? "animate-spin" : ""}`}/>再読み込み</button>
    </div>
    <p className="flex items-center gap-2 text-lg text-muted-foreground"><ShieldCheck className="size-5"/>実際の出番表への反映は、大会本部ログイン設定後に認証済み操作として有効になります。</p>
