...
type EntryRow={entry_id:string;is_op:boolean|null;result_eligible:boolean;competition_id:string;competition_no:string;start_order:number;status:string;rider_id:string;rider_name:string;horse_id:string;horse_name:string;organization_name:string}
...
function normalizeRequestRow(row:RequestRow):AppRequest|null{
 ...
 const base={id:row.id,type,status,createdAt:str(p.createdAt)||row.created_at,orgId,visitorOrgId:str(p.visitorOrgId),visitorName:str(p.visitorName),officialEntryId:status!=="pending"?(row.entry_id||row.original_entry_id||undefined):undefined,cancelledSeedEntryId:str(obj(p._cancellation).seedEntryId)||undefined,cancelledByAdmin:Boolean(obj(p._cancellation).at),fee:feeFromPayload(p,total)} as AppRequest
...
export async function verifyAdminSession(session:AdminSession):Promise<AdminSession>{
 ...
 const user=await response.json() as {email?:string;app_metadata?:{role?:string}}
