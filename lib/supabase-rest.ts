...
function normalizeRequestRow(row:RequestRow):AppRequest|null{
 ...
 const base={id:row.id,type,status,createdAt:str(p.createdAt)||row.created_at,orgId,officialEntryId:status==="reflected"?(row.entry_id||row.original_entry_id||undefined):undefined,fee:feeFromPayload(p,total)} as AppRequest
