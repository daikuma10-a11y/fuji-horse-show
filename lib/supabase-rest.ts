...
function normalizeRequestRow(row:RequestRow):AppRequest|null{
 ...
 const base={id:row.id,type,status,createdAt:str(p.createdAt)||row.created_at,orgId,officialEntryId:status!=="pending"?(row.entry_id||row.original_entry_id||undefined):undefined,cancelledSeedEntryId:str(obj(p._cancellation).seedEntryId)||undefined,cancelledByAdmin:Boolean(obj(p._cancellation).at),fee:feeFromPayload(p,total)} as AppRequest
