...
export async function reorderEntries(competitionId:string,entryIds:string[],accessToken:string):Promise<void>{const response=await fetch(`${SUPABASE_URL}/rest/v1/rpc/reorder_entries`,{method:"POST",headers:{apikey:SUPABASE_PUBLISHABLE_KEY,Authorization:`Bearer ${accessToken}`,"Content-Type":"application/json"},body:JSON.stringify({p_competition_id:competitionId,p_entry_ids:entryIds})});if(!response.ok){let detail="";try{const body=await response.json() as {message?:string};detail=body.message?`: ${body.message}`:""}catch{}throw new Error(`出番順の保存に失敗しました (${response.status})${detail}`)}}
