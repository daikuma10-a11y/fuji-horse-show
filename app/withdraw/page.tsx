...
type Step = "date" | "competition" | "entry" | "confirm"

export default function WithdrawPage() {
  const router=useRouter()
  const [submitting,setSubmitting]=useState(false),[submitError,setSubmitError]=useState("")
  const { getPlayer, getHorse, getOrg, stageWithdraw } = useStore()
  ...
  async function confirmWithdraw(){if(submitting||!entry||!competition)return;setSubmitting(true);setSubmitError("");try{stageWithdraw({entryId:entry.id,competitionId:competition.id,playerId:entry.playerId,horseId:entry.horseId,organizationId:entry.organizationId});router.push("/reception-review")}catch(error){setSubmitError(error instanceof Error?error.message:"受付一覧に追加できませんでした")}finally{setSubmitting(false)}}
  ...
  if(step==="confirm"&&competition&&entry){const player=getPlayer(entry.playerId),horse=getHorse(entry.horseId),org=entry.organizationId?getOrg(entry.organizationId):horse?getOrg(horse.orgId):undefined;return <StepShell subtitle={subtitle} title="この内容で棄権します" description="内容を確認して受付一覧に追加します。確定と保存は一覧画面で行います。" onBack={()=>setStep("entry")} footer={<ActionButton variant="danger" disabled={submitting} onClick={()=>void confirmWithdraw()}>{submitting?"一覧に追加中…":"受付一覧に追加"}</ActionButton>}>
