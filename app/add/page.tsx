...
export default function AddPage(){
 const {getPlayer,getHorse,getOrg,submitAdd}=useStore(); const [step,setStep]=useState<Step>("date"),[date,setDate]=useState<CompetitionDate|null>(null),[competition,setCompetition]=useState<Competition|null>(null),[playerId,setPlayerId]=useState(""),[horseId,setHorseId]=useState(""),[note,setNote]=useState(""),[picker,setPicker]=useState<Picker>(null); const subtitle="追加のお申し込み"; const player=getPlayer(playerId),horse=getHorse(horseId),org=horse?getOrg(horse.orgId):undefined,fee=competition?calcAddFee(competition):null
 if(step==="done")return <CompletionScreen subtitle={subtitle} message="追加エントリーの受付が完了しました。"/>
 ...
 if(step==="competition"&&date)return <StepShell subtitle={subtitle} title="競技を選んでください" description="追加でエントリーする競技を選びます" onBack={()=>setStep("date")}><CompetitionList date={date} onSelect={c=>{setCompetition(c);setPlayerId("");setHorseId("");setStep("input")}}/></StepShell>
 ...
 if(step==="input"&&competition)return <StepShell subtitle={subtitle} title="追加する内容を入力してください" description={`競技 ${competition.number}：${competition.name}`} onBack={()=>setStep("competition")} footer={<ActionButton disabled={!playerId||!horseId} onClick={()=>setStep("confirm")}>確認画面へすすむ</ActionButton>}>
  <div className="flex flex-col gap-3"><PickRow label="選手" value={player?.name??"未選択"} filled={!!player} onClick={()=>setPicker("player")} onClear={()=>setPlayerId("")}/><PickRow label="馬" value={horse?.name??"未選択"} hint={org?`所属団体：${org.name}`:undefined} filled={!!horse} onClick={()=>setPicker("horse")} onClear={()=>setHorseId("")}/></div>
  {(playerId||horseId)&&<button type="button" onClick={()=>{setPlayerId("");setHorseId("")}} className="mt-4 flex min-h-14 w-full items-center justify-center gap-2 rounded-xl border-2 border-border bg-card px-4 text-xl font-bold text-foreground"><RotateCcw className="size-6"/>人馬の選択をすべて解除</button>}
 ...
 if(step==="confirm"&&competition&&player&&horse&&fee)return <StepShell subtitle={subtitle} title="この内容で追加します" description="よろしければ「追加を確定する」を押してください" onBack={()=>setStep("input")} footer={<ActionButton onClick={()=>{submitAdd({competitionId:competition.id,playerId,horseId,note});setStep("done")}}>追加を確定する</ActionButton>}>
  <button type="button" onClick={()=>{setPlayerId("");setHorseId("");setStep("input")}} className="mb-4 flex min-h-14 w-full items-center justify-center gap-2 rounded-xl border-2 border-border bg-card px-4 text-xl font-bold"><RotateCcw className="size-6"/>人馬を選び直す</button>
